...
# TRIGGER_{cycle_group}_DW:
#     TRIGGER_{cycle_group}_DW_FCSO:
#         FCSO_{cycle_group}_{theme_name}:
#             FLG
#             CSO_{cycle_group}_{theme_name}{sub_group}:
#                 CVT
#                 SDB
#                 ODB
#     TRIGGER_{cycle_group}_DW_PDB:
#         PDB_{cycle_group}_{theme_name}
#     TRIGGER_{cycle_group}_DW_CDB:
#         CDB_{cycle_group}_{theme_name}
#     TRIGGER_{cycle_group}_DW_DDB:
#         DDB_{cycle_group}_{theme_name}

PROCESS_RELATIONS = {
    'CVT': ['PROCESS_CSO', 'CSO_{cycle_group}_{theme_name}{sub_group}', '{cycle_name}_{theme_name}{sub_group}贴源层作业流'],
    'ODB': ['PROCESS_CSO', 'CSO_{cycle_group}_{theme_name}{sub_group}', '{cycle_name}_{theme_name}{sub_group}贴源层作业流'],
    'SDB': ['PROCESS_CSO', 'CSO_{cycle_group}_{theme_name}{sub_group}', '{cycle_name}_{theme_name}{sub_group}贴源层作业流'],

    'PROCESS_CSO': ['PROCESS_FCSO', 'FCSO_{cycle_group}_{theme_name}', '{cycle_name}贴源层作业流'],
    'FLG': ['PROCESS_FCSO', 'FCSO_{cycle_group}_{theme_name}', '{cycle_name}贴源层作业流'],
    'PROCESS_FCSO': ['TRIGGER', 'TRIGGER_{cycle_group}_DW_FCSO', '{cycle_name}贴源层触发做业组'],

    'PDB': ['PROCESS_PDB', 'PDB_{cycle_group}_{theme_name}', '{cycle_name}_{theme_name}整合层作业流'],
    'PROCESS_PDB': ['TRIGGER', 'TRIGGER_{cycle_group}_DW_PDB', '{cycle_name}_{theme_name}整合层触发做业组'],

    'CDB': ['PROCESS_CDB', 'CDB_{cycle_group}_{theme_name}', '{cycle_name}_{theme_name}汇总层作业流'],
    'PROCESS_CDB': ['TRIGGER', 'TRIGGER_{cycle_group}_DW_CDB', '{cycle_name}_{theme_name}汇总层触发做业组'],

    'DDB': ['PROCESS_DDB', 'DDB_{cycle_group}_{theme_name}', '{cycle_name}_{theme_name}数据下发作业流'],
    'PROCESS_DDB': ['TRIGGER', 'TRIGGER_{cycle_group}_DW_DDB', '{cycle_name}_{theme_name}数据下发触发做业组'],

    'TRIGGER': ['HEAD', 'TRIGGER_{cycle_group}_DW', 'DW_{cycle_name}触发做业组'],
}


# 作业流数量过多时会导致占用大量线程, 在1.3.6弃用, 由于结构过于复杂后续版本仍弃用
...
def get_process_relation(group, job_name, theme_name, cycle_group):
    ...
    sub_group_range = SUB_GROUP_THEME.get(theme_name)
    cycle_name = CYCLE_NAMES[cycle_group]
    if sub_group_range:
        sub_group = '_'+job_name[sub_group_range[0]:sub_group_range[1]]
    else:
        sub_group = ''
    if process_relation:
        process_relation = process_relation.copy()
        process_relation[1] = process_relation[1].format(
            job_name=job_name, theme_name=theme_name, cycle_group=cycle_group, sub_group=sub_group, cycle_name=cycle_name)
...
def gen_dag():
    ...
    for s_row in general_scripts.etl_jobs:
        group_name = s_row[0]
        if not GROUP_FILTER[group_name]:
            continue
        theme_name = s_row[1]
        job_name = s_row[2]
        description = s_row[3]
    ...
    for d_row in general_scripts.etl_dependencies:
        ...
        #     stop=True
        # ...<作业主题<作业层<周期组
        # ∴作业主题内可通过DAG依赖: level < 3, level >= 3 时通过dependent任务建立依赖关系
